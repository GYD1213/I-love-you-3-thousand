...
- **属性值前面**，**冒号后面**，保留一个空格
- **选择器（标签）和  {  中间**，保留一个空格   强制
- 属性名和之后的：之间不允许包含空格，：与属性值之间必须包含空格

### 1.4.4 选择器规范

【强制】当一个rule包舍多个selector时，每个选择器声明必须独占一行。

![image-20221010113518916](mark-img/image-20221010113518916.png)

【建议】选择器的嵌套层级应不大于3级，位置靠后的限定条件应尽可能精确。

![image-20221010113735667](mark-img/image-20221010113735667.png)

### 1.4.5 属性规范

【强制】属性定义必须另起一行。

![image-20221010113828068](mark-img/image-20221010113828068.png)

【强制】属性定义后必须以分号结尾。

![image-20221010113944362](mark-img/image-20221010113944362.png)
...
- **使用 font 属性时，必须按上面语法格式中的顺序书写，不能更换顺序，并且各个属性间以空格隔开** `style weight size family`
